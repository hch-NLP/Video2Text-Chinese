...
def subtitle_frame(video_path, top=200, bottom=100, left=50, right=50, threshold=0.4, bg_mod=1):
    ...
    print('[INFO] file path: ',str(video_path))
    print('[INFO] total frames:', total_frame)
    print('[INFO] resolution:', str(frame_height * frame_width))
    print('[INFO] fps:', fps)
    ...
    while success:
        if index % 8 == 0:
            ...
            last_roi = roi
            pbar.set_postfix_str(str(index/total_frame))
            pbar.update(8)
    ...
    print('[PROCESS] '+str(len(key_frame_index_list))+' keyframes index stored')
